...
WHERE countrylanguages.language = 'Italian' AND countrylanguages.percentage = 100.0;
...
WHERE countries.name = 'San Marino' AND countries.name != cities.name;

-- Clue #5: Oh no, she pulled a switch – there are two cities with very similar names, but in totally different parts of the globe! She's headed to South America as we speak; go find a city whose name is like the one we were headed to, but doesn't end the same. Find out the city, and do another search for what country it's in. Hurry!
-- Heading to South America(continent(in countries table))
-- Find city name (LIKE)
-- Write SQL query here
SELECT cities.name
FROM cities
JOIN countries ON cities.countrycode = countries.code
WHERE countries.continent = 'South America' AND cities.name LIKE 'Serr%';
...
-- print capital
-- get country capital with city (maybe filter on continent)

-- Write SQL query here
SELECT countries.capital, countries.name
FROM countries
JOIN cities ON countries.code = cities.countrycode
WHERE countries.continent = 'South America' AND cities.name = 'Serra';
...
-- print country 
-- filter by population = 91.084

SELECT cities.name, cities.district
FROM countries
